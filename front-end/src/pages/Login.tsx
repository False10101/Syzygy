...
import { useAuth } from '../context/AuthContext'; // Assuming this path exists
...
export default function Login() {
  ...
  // Mock auth for display purposes if you don't have the context running yet
  const { login } = useAuth(); 
//   const login = async (e: string, p: string) => new Promise(resolve => setTimeout(resolve, 1000));
  ...
  return (
    ...
    <div className="min-h-screen bg-[#030712] flex flex-col justify-center items-center p-4 relative overflow-hidden font-sans selection:bg-indigo-500/30">
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[1000px] h-[500px] bg-indigo-500/10 rounded-full blur-[100px] pointer-events-none"></div>
      <div className="absolute bottom-0 right-0 w-[500px] h-[500px] bg-purple-500/5 rounded-full blur-[80px] pointer-events-none"></div>

      {/* LOGIN CARD */}
      <div className="w-full max-w-[420px] relative group z-10">
        ...
        <div className="relative w-full bg-[#0B0F19]/80 backdrop-blur-xl border border-white/10 rounded-2xl p-8 shadow-2xl">
          ...
          <div className="text-center mb-10">
            <div className="relative inline-block mb-6">
              ...
              {/* Removed: border, bg-color, shadow */}
              {/* Kept: h-24 w-24 and overflow-hidden to crop the whitespace */}
              <div className="relative h-28 w-28 mx-auto flex items-center justify-center overflow-hidden rounded-full">
                <img 
                  src={Logo} 
                  alt="App Logo"
                  // Scale up to zoom in, drop-shadow for pop
            ...
            <p className="text-indigo-200/50 text-sm mt-2">Enter credentials to access the terminal</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between items-center ml-1">
                 <label className="block text-[10px] font-bold text-indigo-200/60 uppercase tracking-widest">Password</label>
            ...
            <button 
              type="submit"
              disabled={isLoading}
              className="w-full group relative overflow-hidden bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-medium py-3 rounded-xl transition-all shadow-[0_0_20px_rgba(79,70,229,0.3)] hover:shadow-[0_0_25px_rgba(79,70,229,0.5)] active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed mt-2"
            >
          ...
          <div className="mt-8 pt-6 border-t border-white/5 text-center">
            <p className="text-sm text-gray-500">
              ...
              <Link to="/register" className="text-indigo-400 font-medium hover:text-indigo-300 transition-colors relative group/link">
                Sign up
